...
int main(void)
{
  ...
  const float host_result = std::accumulate(h_input.begin(), host_input.end(), 0.0f);
  std::cerr << "Host sum: " << host_result << std::endl;

  // move input to device memory
  float *d_input = 0;
  cudaMalloc((void**)&d_input, sizeof(float) * num_elements);
  cudaMemcpy(d_input, &host_input[0], sizeof(float) * num_elements, cudaMemcpyHostToDevice);
